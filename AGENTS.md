...
- 用户于 2026-09-16 授权提交解包怪物模板资源：仅 `resources/monster_library/` 下经筛选的 Mob 站立/移动/飞行 PNG、最小元数据与名称索引可随仓库发布；不得上传完整解包数据库、其他资源或个人采集素材。此库为默认查询目录，配置保存项目相对路径，旧默认目录兼容迁移，不改其他自选路径。
...
- **后台截图 DPI（2026-09-16）**：NewMaple 在 150% 缩放下，原 PrintWindow 将 1920×1080 内容画进 2880×1620 物理客户区缓冲，造成固定缩放错位。截图 worker 使用目标窗口 DPI 上下文获取原生客户区，finally 恢复线程上下文后等比例转换为校准尺寸；不得猜测非黑内容边界或硬编码 1.5 倍。尺寸竞态、非等比和返回格式异常必须拒绝并关闭辅助进程，保留超时及禁止桌面回退规则，不修改主进程/游戏 DPI 或用户校准。

- **怪物双向匹配（2026-09-16）**：仅主循环怪物检测显式设置 `mirror_horizontal=True`。Template 的图像与 Alpha 在内存水平镜像并缓存，名称保持不变以兼容分类/过滤，双向候选统一 NMS，不降低阈值、不更改素材计数或原文件；人物模板和过滤项默认不镜像。镜像缓存随模板重载释放，不每帧重建；仍共用同帧 SceneFeatures。

- **采集预览 Debug（2026-09-16）**：`capture_keep_debug` 默认关闭，面板持久保存；开启时通过限定作用域的 HUD 快照在交互 Canvas 叠加框，不改冻结图像像素，不把框写入模板。放大小地图必须按源 ROI 到预览 ROI 映射；取消/异常恢复上下文，不能影响 F7/单项显隐或下次会话。
- **龙咆哮共享定点（2026-09-16）**：直接使用公共 `platform_center`，删除独立定点采集要求和按钮；旧 `dragon_roar_point` 不迁移、不覆盖平台安全点，也不作为回退。平台安全点失效时必须重采公共点。

- **血蓝条按需采集（2026-09-16）**：挂机基础校准仅依赖小地图、玩家标记和战斗区，不要求血蓝条；策略依赖仍单独检查。`status_regions_complete` 保留完整状态区汇总含义，但不能再作为基础启动条件。全局自动喝药开启时仍要求血蓝条均采集，不能从默认区域误发药键；保留旧配置和采集数据。

- **本地素材倍率（2026-09-15）**：用户要求本地库素材默认按 1.5 倍使用。在 `load_templates` 中按完整 `local-名称-ID-SHA256.png` 命名协议识别，图像和 Alpha 最近邻同步放大；磁盘原图不改、重载不累乘，不影响截图素材和全局检测缩放。

- **本地怪物库（2026-09-15）**：`local_monsters.py` 只读已解包的 SQLite/JSON/PNG，不执行资源目录中的 Python。NewMaple 档案从本地搜索并导入当前分类，经典档案阻止混用。Data/BetaData 显式分开；索引路径必须留在库内，引用要检测循环与跨数据集。只导入 stand/move/fly 动作，保留 Alpha、裁透明空边；内容哈希去重、独占发布，不覆盖已有素材。对话框后台读库，Tk 更新仅主线程，沿用暂停视觉/重载模板和管理回收流程；不为导入启动游戏、截图或发键。

- **指定 HWND（2026-09-15）**：用户从窗口名字列表选择，不显示句柄/PID/路径；同名追加编号，同一候选集在不同面板按身份排序编号一致。定位按钮仅有限次闪烁标题栏和任务栏，不切焦点或发键。直接建立 HWND/PID 会话，不按标题搜索或回退；刷新保留具体身份，原选择消失时清空选择，不选同名替代。无效/本进程/最小化窗口拒绝连接；复用安全切换与取消流程，不持久保存句柄。编号在候选增减后可变化，应重新定位；不能将选窗隔离宣传为全局输入或共享配置的多实例隔离。
...
- **头部恢复与黑白姓名板（2026-09-12）**：用户授权优化本人已确认后的头部接替。头部粗检无候选时，仅旧位置附近追加原分辨率复核，仍用原头部门槛。仅 `known_identity_aux` 恢复分支可使用独立 `HeadRecoveryWindow`：0.9 秒内至少 3 次真实头部命中、最多 1 个漏帧，固定标记位移 ≤0.5 像素、固定背景稳定、头部固定位置偏差 ≤4–12 像素；歧义/移动/尺寸改变/超时立即清空，漏帧不得输出旧框，姓名板和首次辅助身份仍保持严格连续帧确认。暂停、配置和真实视觉记录清除此窗口。姓名板显式开启无色差模板的灰度替代通道，非零色差模板与所有非姓名板调用保持原颜色通道，后续字形身份和空间门禁不变。

- **定位失败留证**：`localization_diagnostics` 是只读观察器，默认开启，可由 `diagnostics.player_localization=false` 在重启后关闭。复用同帧匹配分数、ROI 和候选，不为诊断追加扫描、不放宽身份/连续帧/小地图安全门禁。挂机期间连续真实视觉缺失至少 2 秒才留图；异步有界队列、每实例 60 组及目录 256 MiB 上限，日志故障不得中断输入。截图只保存战斗区和小地图；暂停单独结案，不能把暂停时段报成挂机卡死。新增诊断字段需保留坐标空间、扫描倍率、首个失败状态及原始候选身份分，不能用过滤后的数量推断所有原始候选都低分。

- **狩猎验证人工提醒**：2026-09-16 用户要求动态分辨率及可选检测后停机，覆盖此前仅响铃限制。默认扫描实时游戏客户区，旧默认左下区域自动迁移，其他自定义区域保留；原生字形与当前分辨率等比字形共存，缓存有界，窗口尺寸变化清除待确认计数但不清旧消息锁存/冷却。`mbv/verification_alert.py` 仍保持只读、不持有输入对象；仅 NewMaple 使用。总开关及 `pause_on_detect` 均默认关闭、即时保存热更新，勾选本身不能调用 apply_config/disarm。确认命中且显式启用暂停时，由 Bot 在行动锁内先关闭自动喝药、清理待处理启用请求并 disarm 释放按键，再响铃；声音失败不能恢复挂机。仍保留连续 3 次、0.5 秒扫描、10 秒消失及 60 秒冷却；F8 不重置锁存，不读题、不答题、不自动恢复，人工处理后手动启动。
...
- **龙咆哮定点策略（2026-09-15）**：用户确认只用唯一实时小地图玩家标记定位，显式 `localization_mode=minimap`，无需姓名牌/头部及视觉配对，不运行玩家模板匹配、不伪造屏幕锚点。固定战斗识别区内本帧怪物数量严格超过阈值才发独立技能，禁止用 monster hold 凑数量；旧相对角色攻击区域保留但不使用。小地图定点仍必须采集；偏离优先回位，0.6 倍到达容差与超时锁存防止反复动作。标记丢失/歧义立即停战斗和移动，不追怪/拾取/定时走动或盲走；补给按原保护运行。公共层仅对此定位能力放行无方向 cast，保留窗口/权限/补给/取消/冷却及移动验证；其他策略的视觉身份、地图配对期限与地图禁攻击规则不变，不得在策略发键。
