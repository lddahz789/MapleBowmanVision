...
- 用户于 2026-09-05 明确要求新增 `hybrid` 混合后台；2026-09-06 确认 PostMessage 转向无效并要求优化后，实际移动和转向均允许临时激活游戏并使用独立 SendInput 通道。同日用户因混合攻击慢明确同意普通按键按焦点路由：攻击、Buff、喝药、拾取及跳跃攻击的技能键在游戏前台用 SendInput、后台用 PostMessage；不为普通技能主动切窗。同日最新要求：战斗转向不再主动激活，仅实际导航可切前台；详见下方混合后台转向规则。此授权只属于显式选择的混合模式，不改变 `window_message` 的禁止全局输入承诺。
...
2. 面板先启动并等待用户选择、连接窗口，再由 worker 跑 `BowmanBot.run`：`mss` 截客户区 → 裁 HP/MP/小地图/战斗区。旧配置只作为默认候选，不在面板构造时自动连接。
...
- **先启动后选窗**：运行使用明确的会话 `WindowTarget`；面板采集用 `selected_window` 上下文让全部 `find_game_window` 共用所选目标，不能各自按标题重新匹配。上下文退出必须恢复，不能跨档案泄漏。窗口身份按 HWND/PID 复核，失效不可转绑同名窗口；无连接时不能截图或发键。切换先停止旧 worker、关闭喝药并释放按键，旧 worker 完全结束后才创建新 Bot，清除旧人物身份、小地图基准、路线与请求。仅结束连接不能关闭面板/HUD，只有真正退出才 close；HUD `reset_session` 只能在主线程且旧 worker 结束后调用。个人配置只记住 `window.preferred_title` 与 `preferred_executable`，不能持久保存 HWND/PID，不能清空原配置或跨档案复用素材。独立 CLI 保留旧配置找窗兼容行为。

- **清新分页面板**：2026-09-13 用户要求清晰简约、清新配色与按功能横向分页。用户随后要求整体参考苹果/macOS：样式统一在 `mbv/panel_theme.py`、圆角原生控件在 `mbv/panel_widgets.py`，保持中性灰白底、系统蓝主操作、分段页签与常规中文字体；不安装 Apple 字体、不修改窗口过程、不为了装饰增加固定区域高度；七页为窗口连接/区域校准/模板采集/职业策略/按键补给/运行设置/性能监控。2026-09-13 后续要求窗口选择独立成页，狩猎验证开关固定在外层，与运行控制一样切页可见；压缩固定顶栏和底栏占用但不缩小到难读。狩猎验证继续仅响铃热更新，不因挪动开关触发 apply_config 或暂停挂机；各页共用原配置字典及保存链路，切页不得重建Bot或重置输入。滚轮只路由所在页，禁止 `bind_all/unbind_all` 影响其它窗口；动态策略容器只放策略内容，不销毁其它页的设置。`tools/preview_panel.py` 使用临时示例配置并禁用游戏连接，仅供隔离 UI 预览。

- **狩猎验证人工提醒**：2026-09-11 用户明确只要检测聊天字样并响铃，不自动暂停、不识题、不提交。`mbv/verification_alert.py` 必须保持只读观察器，不持有键盘、窗口控制或策略对象；复用成功截图，原生字形模板只用于 NewMaple，classic 不复用。默认关闭，独立开关即时落盘和热更新，不能调用 apply_config/disarm；异步声音不能 sleep 阻塞行动线程。连续命中确认、旧消息锁存和限频独立于 armed，普通 F8 不重置锁存。提示音/识别故障只能停用提醒器并显示错误，日志写失败不能抑制已检测到的提示音或中断挂机。人工输入前由用户自行暂停并关闭自动喝药，不能新增自动验证或自动恢复流程。
...
- **姓名板缩放漏检**：细字在半分辨率下会因一像素位移低于候选门槛。粗检无有效身份时，在同一 ROI、同一 `SceneFeatures` 上追加一次原分辨率复核，门槛不得低于正常姓名板阈值；正常粗检命中或已用原分辨率时不重复扫描。复核候选仍须先校验名字字形再跨模板去重，并经过原 tracker 消歧及连续帧确认；不得借此放宽小地图补位或攻击门禁。
...
- **龙咆哮定点策略**：本帧真实怪物数量严格超过阈值才发独立技能，禁止用 monster hold 凑数量；独立相对角色范围与小地图定点必须采集。偏离优先回位，0.6 倍到达容差与超时锁存防止反复动作；不追怪/拾取/定时走动，完全失定位不盲走（包括首个决策前的旧人物框），仅小地图时不施法。无方向技能使用公共 cast 执行器，保留所有窗口/身份/补给/取消/冷却保护，不得在策略发键。
...
- **混合后台移动**：前台授权与扫描码保持放 `mbv/hybrid_movement.py`，不得在策略中切窗。启动不置顶，后台仍窗口截图；发移动键前检查游戏 HWND/PID/前台，首次激活后等新视觉帧。按键释放后观察空闲 0.7 秒才尝试；用户于 2026-09-06 明确要求鼠标位移不打断挂机，故不再检查光标位移或使用会被位移刷新的 GetLastInputInfo，移动会话限 4 秒、按键无续期限 0.5 秒、行动心跳限 0.8 秒；独立 watchdog 约 20 毫秒检查，暂停/退出信号也必须取消。2026-09-05 恢复旧窗口失败导致崩溃后，用户授权切到前台不再返回旧窗口：移动结束/中断只抬键，不恢复原窗口；用户切窗或操作后不抢回焦点。输入异常应在公共行动边界暂停并提示，不让视觉线程退出；故障期间不继续暂停喝药，须显式启动重新绑定，抬键失败不能清除错误继续移动。焦点操作仍放可终止子进程，不允许无界阻塞。输入竞态仍需用户端实测，不能承诺完全不打断。技能按新授权按焦点路由，但每次按下/抬起必须保持原通道；不能因窗口消息失败回退全局输入。

## 怎么跑

- **路线清怪朝向稳定**：2026-09-06 拾取去程/到点/普通返程清怪，从本帧双向索敌区内的候选优先清当前侧，该侧连续缺失 0.4 秒再换边；恢复原侧、药/Buff/定位中断重置换侧确认，清怪结束清除侧锁，不得缓存旧框继续攻击。策略通过 `face_tap_seconds=0.08` 请求前台稳定转向：仅 foreground 普通攻击消费这个路线覆盖，覆盖每次必要换边/缓存失效，沿用 120 ms 稳定准备和抬键等待，保留用户更长时长，同侧持续攻击不重复。其它模式忽略该攻击时序覆盖，尤其 hybrid 不为转向激活。返程清怪短暂 minimap_only 先按已有视觉宽限停走，超时仍受原导航期限限制；安全返程不延迟。

- **前台回位后恢复攻击**：2026-09-06 用户明确报告拾取/回位刚结束后朝向不一致。仅 foreground 的普通攻击在真实移动发键或拾取键释放后标记需重新转向：先松开拾取与左右键，跨帧至少等待 0.12 秒且目标方向不变，再短按方向至少 0.08 秒（更长个人配置保留），抬键后等待 0.12 秒再攻击；仍满足原至少 0.6 秒停攻准备。换边重置准备，发送失败不能清除标记。不要恢复逐次攻击方向键或固定周期停攻，不得改变 hybrid/background/window_message 的时序和切窗承诺。输入序列不是视觉面向确认，实际效果须实测。

- **混合后台转向**：2026-09-06 用户要求后台攻击不要频繁切前台，覆盖此前允许为转向激活的行为。所有策略的普通攻击、只转向和跳攻均不得为转向主动切窗；游戏在后台时，普通攻击/跳攻只发技能并沿用游戏当前面向，不能发送无效的 PostMessage 方向或伪造朝向缓存，只转向动作延后。HUD 用“后台沿用游戏当前朝向”，日志 direction=null 并分别记录 requested_direction/turn_deferred，不把目标方向当实际朝向。游戏已经在前台时仍使用受保护扫描码转向，begin(allow_focus=False) 必须在输入层再次禁止激活，首次租约等新视觉帧；方向短按尊重 behavior.face_tap_seconds（下限 0.02 秒、混合上限 0.1 秒）。实际周期短步、拾取往返、安全回位及策略要求的实际导航保留原前台授权；后台完全丢失定位时等待，不主动切窗左右找人。其他输入模式行为不得随本项改变。所有模式仍取消每五秒转向刷新；普通药/Buff不单独清方向，方向/跳跃键绑定除外；焦点、身份、定位与抬键保护不变。
- **混合空闲等待与输入复核**：前台也保留按键释放后的 0.7 秒空闲观察，等待状态不是 disarm；HUD 必须区分已经前台与准备切前台。成功发送自有键 keyup 后，仅该键及修饰键别名可短暂排除 80 ms，防止异步键状态滞后误判；不得扩展为忽略所有输入或延长按键保持。其它按键（含鼠标按钮）、焦点变化仍立即中断；纯鼠标位移不阻断或重置空闲观察，原因分开记录，抬键失败不得借宽限清除故障。

- **原地路线拾取**：仅同平台小地图点位；采样与安全点一致，重采小地图失效、重采战斗区保留。用户于 2026-09-06 要求去程与返程均先原地清怪：普通路线清怪期间停走、停拾取，连续无有效目标 0.4 秒后再继续；重现目标、定位/药/Buff 中断重置无目标确认，不把目标消失当作已验证死亡。关闭开关、点位无效、掉层、定位异常的安全返程仍优先回位；暂停立即释放。路线状态必须由运行层按会话保存，不能写入全局策略实例。确认离开并回到安全点且清怪结束才重置周期右移；仅发送过按键不代表拾取成功。2026-09-06 用户要求路线拾取长按：有效去程、到点和返程保持普通拾取键，并仅对此长按键约每 0.1 秒补自动重复 keydown（中途不 keyup），不能把一次 SendInput down 当作完整实体长按。每帧续期、无续期约 0.8 秒独立释放；首次前台 hold 必须在登记期限后启动重复/超时线程，测试要覆盖实际线程，不能只 mock 掉启动器。普通技能/方向键不得因此新增重复。旧 pickup_interval_seconds 仅兼容标识。遇怪/药/Buff/丢定位/暂停/结束/分段必须松开，实际发技能前先释放可能冲突的拾取键。混合到点无移动租约时不得每帧 finish_movement 抬掉拾取；新移动租约前先释放自有拾取键，避免空闲检查自锁。移动叠加拾取仍走普通按键通道，混合后台路线主动分段不得清空无进展监控或取消原 watchdog。定位丢失时导航任务不盲走找人。
